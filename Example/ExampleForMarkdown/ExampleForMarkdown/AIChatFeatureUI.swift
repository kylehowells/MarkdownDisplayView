...
final class AIChatImageStripView: UIView {
    ...
    private func makeItemView(_ item: AIChatSelectedImage) -> UIView {
        ...
        container.insertSubview(statusBackground, belowSubview: statusView)
        ...
        removeButton.accessibilityLabel = "移除图片"
        ...
        NSLayoutConstraint.activate([
            ...
            statusBackground.centerXAnchor.constraint(equalTo: container.centerXAnchor),
            statusBackground.centerYAnchor.constraint(equalTo: container.centerYAnchor),
            statusBackground.widthAnchor.constraint(equalToConstant: 28),
            statusBackground.heightAnchor.constraint(equalToConstant: 28),
            statusView.centerXAnchor.constraint(equalTo: statusBackground.centerXAnchor),
            statusView.centerYAnchor.constraint(equalTo: statusBackground.centerYAnchor),
            statusView.widthAnchor.constraint(equalToConstant: 20),
            statusView.heightAnchor.constraint(equalToConstant: 20),
            removeButton.centerXAnchor.constraint(equalTo: container.trailingAnchor, constant: -2),
            removeButton.centerYAnchor.constraint(equalTo: container.topAnchor, constant: 2),
...
final class AIChatHistoryViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        NSLayoutConstraint.activate([
            ...
            tableView.bottomAnchor.constraint(equalTo: newButton.topAnchor, constant: -8),
    ...
    @objc private func applySelection() {
        onReferenceSelection?(selectedIDs)
        dismiss(animated: true)
    }
...
extension AIChatHistoryViewController: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let reuseIdentifier = "AIChatHistoryCell"
        let cell = tableView.dequeueReusableCell(withIdentifier: reuseIdentifier)
            ?? UITableViewCell(style: .subtitle, reuseIdentifier: reuseIdentifier)
        let conversation = conversations[indexPath.row]
        cell.textLabel?.text = conversation.title
        cell.textLabel?.font = .systemFont(ofSize: 16, weight: .semibold)
        let question = conversation.messages.last(where: { $0.role == .user })?.content ?? "暂无问题"
        cell.detailTextLabel?.text = "\(Self.dateFormatter.string(from: conversation.updatedAt)) · \(question.prefix(48))"
    ...
    func tableView(_ tableView: UITableView, accessoryButtonTappedForRowWith indexPath: IndexPath) {
        onOpenConversation?(conversations[indexPath.row])
        dismiss(animated: true)
    }
